...
export default function ToolBar({
    ...
    const handleMuteAll = () => {
      ...
      return (
        <div className={isMobileWidth ? 'p-2 bg-gray-800 flex justify-between items-center' : 'p-4 bg-gray-900 flex justify-around items-center'}>
          {isMobileWidth ? (
            <>
              <MuteAudioButton toggleAudio={toggleAudio} hasAudio={hasAudio} changeAudioSource={changeAudioSource} />
              <EndCallButton handleEndCall={endCall} />
              <MuteVideoButton toggleVideo={toggleVideo} hasVideo={hasVideo} changeAudioSource={changeAudioSource} />
              {/* Add other buttons as necessary for mobile view */}
            </>
          ) : (
            <>
              <MoreOptionsButton participants={participants} room={room} localParticipant={localParticipant} />
              <MuteAudioButton
      ...
      />
         {VideoExpress.hasMediaProcessorSupport() && <VideoFilterButton room={room} />}
              {/* Add other buttons as necessary for desktop view */}
              <SpeakerSelector room={room} changeAudioOutput={changeAudioOutput} getCurrentAudioOutput={getCurrentAudioOutput} />
...
<ReactionsButton handleMuteAll={handleMuteAll} areAllMuted={areAllMuted} room={room} />
